...
//        String  generatePath = path.getParent() + File.separator + path.getName()+File.separator + path.getName()+System.currentTimeMillis() + "." + format;
//        // 输出流
...
//        out.close();
    	 ...
    	 String generatePath = "";
         if( path.isDirectory() ){
             generatePath = path.getParent().endsWith("/") == false ? path.getParent() + File.separator + path.getName()+System.currentTimeMillis() + "." + format: path.getParent() + path.getName()+System.currentTimeMillis() + "." + format;
         }else {
             generatePath = path.getParent().endsWith("/") == false ? path.getParent() + File.separator : path.getParent();
             generatePath += path.getName().substring(0,path.getName().lastIndexOf("."))+System.currentTimeMillis() + "." + format;
         }
         // 输出流
         FileOutputStream outputStream = new FileOutputStream( generatePath );
         // 压缩输出流
     ...
     * @throws Exception
     */
    public static ZipOutputStream zipFileStream(File path,String format) throws Exception {
        String generatePath = "";
        if( path.isDirectory() ){
  ...
  //需要注意的是当删除某一目录时，必须保证该目录下没有其他文件才能正确删除，否则将删除失败。
    ...
    public static void main(String[] args) {
        String path = "D:\\eclipse\\springboot\\cx-luban-tool\\src\\cn\\com\\cx\\luban\\core\\festival_activity";
        String format = "zip";
        try {
